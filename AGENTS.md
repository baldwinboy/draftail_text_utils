...
draftail_text_utils is a Django/Wagtail package. The source code is in `src/draftail_text_utils/`, with tests in `tests/` and a demo Wagtail site in `demo/`.
...
- Tests: pytest with pytest-django. Test settings are in `src/draftail_text_utils/test/settings.py`.
...
```
src/draftail_text_utils/       # Package source code
src/draftail_text_utils/test/  # Django test app with settings, URLs
